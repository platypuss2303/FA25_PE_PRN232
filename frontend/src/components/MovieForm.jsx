...
export default function MovieForm({ initialData = {}, onSubmit, loading, isEdit = false, validationErrors = {} }) {
  ...
  const [imagePreview, setImagePreview] = useState(initialData.posterImageUrl || null)
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-lg shadow-soft rounded-3xl p-8 border border-gray-100">
      {/* Title Field */}
      <div className="mb-6">
        <label className="block text-gray-800 text-sm font-bold mb-2 flex items-center gap-2" htmlFor="title">
        ...
        <input
          ...
          className={`w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all ${
            validationErrors.Title ? 'border-danger-500 bg-danger-50' : 'border-gray-200 bg-white'
          }`}
        />
      ...
      {/* Genre Field */}
      <div className="mb-6">
        <label className="block text-gray-800 text-sm font-bold mb-2 flex items-center gap-2" htmlFor="genre">
          <span className="text-lg">🎭</span>
          Genre
        ...
        <select
          ...
          onChange={handleChange}
          className={`w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all ${
            validationErrors.Genre ? 'border-danger-500 bg-danger-50' : 'border-gray-200 bg-white'
          }`}
        >
      ...
      {/* Rating Field */}
      <div className="mb-6">
        <label className="block text-gray-800 text-sm font-bold mb-2 flex items-center gap-2" htmlFor="rating">
          <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
          Rating
          <span className="text-xs font-normal text-gray-500">(1-5, Optional)</span>
        </label>
        <div className="flex items-center gap-4">
          <input
            ...
            className={`w-32 px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all ${
              validationErrors.Rating ? 'border-danger-500 bg-danger-50' : 'border-gray-200 bg-white'
            }`}
          />
          {formData.rating && (
            <div className="flex gap-1">
              {[...Array(5)].map((_, index) => (
                <Star
                  key={index}
                  size={20}
                  className={index < formData.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}
                />
              ))}
            </div>
          )}
      ...
      <div className="mb-6">
        ...
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all ${
              imageFile ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'border-gray-200 bg-white'
            } ${validationErrors.PosterImageUrl ? 'border-danger-500 bg-danger-50' : ''}`}
          />
